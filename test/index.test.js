import { resulti, isResulti, resultify, ok, err } from "../src/index";
...
describe("resulti", () => {
  ...
  it("unwrapOr will return type variant, unless resulti is error then it will return whatever is passed to it", () => {
    ...
    expect(resulti(undefined, "IsError").unwrapOr("OtherValue")).toBe(
      "OtherValue"
    );
  });
  ...
  it("unwrapOrElse will return type variant, unless resulti is error then it will call error with func passed to it", () => {
    ...
    expect(resulti(undefined, "IsError").unwrapOrElse(x => x.length)).toBe(
      "IsError".length
    );
  });
  ...
  it("expect will return type variant, unless resulti is error then it throw value passed to it", () => {
    ...
    expect(() => resulti(undefined, "IsError").expect("OtherValue")).toThrow(
      "OtherValue"
    );
  });
  ...
  it("expect will return error, unless resulti is ok then it throw value passed to it", () => {
    ...
    expect(resulti(undefined, "IsError").expectErr("OtherValue")).toBe(
      "IsError"
    );
  });
  ...
  it("mapOrElse will return new resulti with type variant mapped with initial mapping func or error mapped with secondary func", () => {
    ...
    const mappedErr = resulti(undefined, "IsError").mapOrElse(
      x => x.length,
      x => x.split("")
    );
  ...
  it("andThen will perform operation on type variant or return error variant", () => {
    ...
    expect(resulti(undefined, "IsError").andThen(x => x.length)).toBe(
      "IsError"
    );
  });
  ...
  it("or will return passed value if resulti is error otherwise will return resulti", () => {
    ...
    expect(resulti(undefined, "IsError").or("RandomValue")).toEqual(
      "RandomValue"
    );
  });
  ...
  it("orElse will perform operation on error variant or return type variant", () => {
    ...
    expect(resulti(undefined, "IsError").orElse(x => x.length)).toBe(
      "IsError".length
    );
  });

  it("will never mutate below anyone's feet", () => {
    const result = resulti("IsOk");
    const firstMappedOk = result.map(x => x.length);
    const secondMappedOk = result.map(x => x.split(""));

    expect(result.unwrap()).toBe("IsOk");
...
describe("resultify", () => {
  ...
  it("allows mapping the promise if it rejects using the third argument", () => {
    const resultiedPromise = resultify(
...
describe("ok", () => {
  it("returns a resulti with ok variant", () => {
...
describe("err", () => {
  it("returns a resulti with an err variant", () => {
